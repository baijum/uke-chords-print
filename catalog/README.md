...
- `popular_chords.txt`, `challenging_chords.txt`, and `all_chords.txt` (Fm7 only) pin some explicit voicings, marked `@tuning standard`. With `--tuning baritone` or `--tuning d-tuning`, those chords fall back to generated voicings for that tuning (distinct ones where a chord is pinned with different shapes).
