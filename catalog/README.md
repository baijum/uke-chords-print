...
- `popular_chords.txt` and `challenging_chords.txt` pin some explicit voicings, marked `@tuning standard`. With `--tuning baritone` or `--tuning d-tuning`, those chords fall back to the generated voicing for that tuning.
